...
namespace Stratego
{
	...
	public class JeuRegles : ISerializable
	{
		...
		{
			if(cheatMode && personnage.Equipe == Personnage.Rouge && personnage is Drapeau)
		...
		public void VisibiliteDrapeauRouge(Graphics graphics, bool etat)
		{
			Personnage personnage;
			...
			for (int id = 0; id < positionPieces.Count; id++)
			{
				...
				if (personnage.Equipe == Personnage.Rouge && personnage is Drapeau) // ne dessine que les pièces valides
				{
					if(etat)
						graphics.DrawImage(ImagePiece(personnage), positionPieces[id].Rect);
					else
						graphics.DrawImage(imagePieceAdverse, positionPieces[id].Rect);
				}
			}
		}
